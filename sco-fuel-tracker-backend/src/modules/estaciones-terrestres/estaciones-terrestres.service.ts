...
@Injectable()
export class EstacionesTerrestresService {

    constructor(private readonly httpService: HttpService) {}

    async getEstacionesTerrestres(): Promise<GetEstacionesTerrestresResponse> {
        try {
            const response: any = await this.httpService.get('https://sedeaplicaciones.minetur.gob.es/ServiciosRESTCarburantes/PreciosCarburantes/EstacionesTerrestres/')
            .toPromise()
            .then(res => res.data)
            ...
            if (response.ListaEESSPrecio && response.ListaEESSPrecio.length > 0) {
                ...
                for (const estacionTerrestre of response.ListaEESSPrecio) {
                    let id: string = `${estacionTerrestre.Latitud}_${estacionTerrestre["Longitud (WGS84)"]}_${estacionTerrestre["C.P."]}_`;
                    id = id + `${estacionTerrestre.IDMunicipio}_${estacionTerrestre.IDProvincia}_${estacionTerrestre.IDCCAA}`;

                    const posteMaritimoDto: EstacionTerrestreDto = {
                        id_estacionTerrestre: `${id}`,
                ...
                estacionesTerrestresResponse.TotalEstaciones = estacionesTerrestresList.length;
            }
